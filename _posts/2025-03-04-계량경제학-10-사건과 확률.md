---
title: 계량경제학 10 - 사건과 확률
date: 2025-03-04 12:00:00 +0900
categories: [경제학, 계량경제학]
tags: [econometrics, probability]
author: rachihyeon 
description: 여러 변형 OLS에 앞서 필요한 조건, 조건부 확률, 조건부 기댓값, 확실한 수렴(Sure Convergence), 거의 확실한 수렴(Almost Sure Convergence), 확률 수렴(Convergence in Probability), 분포 수렴(Convergence in Distribution), 큰 수의 법칙, 중심 극한 정리 등의 수리통계학 내용을 다루고, Classical assumption에서 오차항의 정규성이 없을 때도 ols estimator가 정규성을 따르는지 확인해본다.
# comments: 
# media_subpath: 
# pin: true
math: true
...
우리는 [Gauss Markov Theorem](/posts/계량경제학-5-Gauss-Markov-Theorem/#2-1-classical-assumption)에서 몇 가지 가정을 했었다.
...
[위에서 조건부 기댓값을 적용한 가정들](/posts/계량경제학-10-사건과-확률/#1-3-classical-assumption)을 새로 했는데, 이 가정 하에서 OLS estimator의 속성에 대해서 확인을 해보겠다.
...
<br>
[큰 수의 법칙](/posts/계량경제학-10-사건과-확률/#2-6-law-of-large-numbers큰-수의-법칙)에 의해,
\\
$$
\begin{align}
...
인데, [위 가정들](/posts/계량경제학-10-사건과-확률/#3-1-근사를-위한-가정들)을 기반으로 [중심극한정리](/posts/계량경제학-10-사건과-확률/#2-7-central-limit-theorem중심-극한-정리)를 이용하면,

$$
\frac{1}{\sqrt{T}}\sum _{t=1}^{T} X_t\epsilon_t \overset{d}{\to} N(0, \sigma^2Q)
$$

근데 모분산은 알려져있지 않은 경우가 많다. 따라서 모표준편차를 대체할 값이 필요한데, [검정에 대해 다룬 포스트](/posts/계량경제학-6-Test/#3-1-불편량)에서 사용했던 방법을 사용하겠다.
