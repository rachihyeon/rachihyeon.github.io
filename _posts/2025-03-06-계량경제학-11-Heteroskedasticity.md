---
title: 계량경제학 11 - 이분산성(Heteroskedasticity)
...
이 얘기를 왜 했냐하면... 우리가 [OLS 추정량을 계산할 적에 했던 가정들](/posts/계량경제학-5-Gauss-Markov-Theorem/#2-1-classical-assumption)에서 등분산성 조건을 세웠지만, 실제 연구에서는 그렇지 않은 경우가 많다.
...
우리는 [지난 표스트에서 분포 근사](/posts/계량경제학-10-사건과-확률/#3-2-분포-근사)에 대해서 알아 보았다.
...
>3. [분포 근사](/posts/계량경제학-10-사건과-확률/#3-2-분포-근사)를 이용한 검정통계량 $$\frac{\hat{\alpha}}{s.e.(\hat{\alpha})} \sim N(0, 1)$$를 사용한다. (여기서 $$s.e.$$는 표준 오차를 의미한다.)
{: .prompt-info}
<br>
