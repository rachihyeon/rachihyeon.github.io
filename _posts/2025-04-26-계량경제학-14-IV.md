---
title: 계량경제학 14 - IV(Instrument Variable) estimation
date: 2025-04-26 19:00:00 +0900
categories: [경제학, 계량경제학]
tags: [econometric, iv, endogenous, exogenous, 2sls, 내생성, 외생성]
author: rachihyeon 
description: Input data가 알려지지 않은 속성에 의해 오차항과의 상관관계가 존재할 때 output을 정확하게 예측해낼 수 있는 방법인 도구변수 추정법에 대해 알아본다.
# comments: 
# media_subpath: 
# pin: true
math: true
...
IV estimation(도구 변수 추정법)이란, 설명변수($$X_t$$)에 [내생성](/posts/계량경제학-14-IV/#1-1-endogenous내생성)이 존재할 때 발생하는 치명적인 결함을 치유하고자 오차항($$\epsilon$$)과 직교하고 설명변수와 상관관계가 있는 도구변수($$W$$)를 사용하여 $$Y$$를 설명하려는 방법을 말한다.
...
이전에 [Gauss Markov Theorem](/posts/계량경제학-5-Gauss-Markov-Theorem/#1-projection)을 다뤘을 때 projection 개념을 사용했던 것을 기억할 것이다.
